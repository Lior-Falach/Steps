...
def _generate_example_linear_angular_speed(t):
  ...
  time_points = (0,5,30)
  speed_points = ((0, 0, 0, 0), (vx, 0, 0, 0), (vx, 0, 0, 0))
...
class RobCom:
    def __init__(self):
        ...
        self.ros_pub_lowstate = rospy.Publisher("/low_state_Hi_res", A1LowState, queue_size=1)  # topic, msg, queue=1=>publish every time
        self.ros_pub_GT = rospy.Publisher("/A1_GT_Hi_res", A1True,queue_size=1)  # topic, msg, queue=1=>publish every time
        rospy.loginfo("> lowsatet publisher corrrectly initialized")
...
def main(argv):
  ...
  while not rospy.is_shutdown():
    ...
    pos, ori =robot.GetBasePose()

    q, dq, ddq, tau = [], [], [], []
    q1 = robot.GetMotorAngles()
    dq1 = robot.GetMotorVelocities()
    ddq1 = robot.GetMotorVelocities()
    tau1 = robot.GetMotorTorques()
    ff1 = np.multiply(robot.GetFootContacts(),1)
    for m in range(12):
        q.append(q1[m])
        dq.append(dq1[m])
        ddq.append(ddq1[m])
    ...
    # ddq = robot.GetMotorVelocities()
    # tau=robot.GetMotorTorques()

    Ro.ros_pub_lowstate.publish(*[q, dq, ddq, tau,ff,[0.1,0.1,0.1,0.1], robot.GetBaseRollPitchYawRate(), robot.GetBaseVelocity()])
    Ro.ros_pub_GT.publish(*[pos,ori])
